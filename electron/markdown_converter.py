...
def convert_image_links(markdown_text, file_path):
    ...
    # Also match standard markdown image syntax
    standard_pattern = r'!\[(.*?)\]\((.*?)\)'
    def replace_standard_image(match):
        alt_text = match.group(1)
        image_path = match.group(2)
        return replace_image(type('Match', (), {'group': lambda x: image_path}))
    
    processed = re.sub(standard_pattern, replace_standard_image, processed)
